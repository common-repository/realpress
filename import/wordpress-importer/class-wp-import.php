...
/**
 * REALPRESS_Import class.
 */
class REALPRESS_Import extends WP_Importer {
	...
	function dispatch() {
		$this->header();
		$step = Validation::sanitize_params_submitted( $_GET['step'] ?? 0 );
		$step = (int) $step;
		...
		switch ( $step ) {
			...
			case 2:
				...
				$fetch_attachment        = Validation::sanitize_params_submitted( $_POST['fetch_attachments'] );
				$this->fetch_attachments = ( ! empty( $fetch_attachment ) && $this->allow_fetch_attachments() );
				$this->id                = (int) Validation::sanitize_params_submitted( $_POST['import_id'] );
	...
	function get_author_mapping() {
		$imported_authors = Validation::sanitize_params_submitted(
			$_POST['imported_authors'] ?? ''
		);
		...
		foreach ( (array) $imported_authors as $i => $old_login ) {
			...
			$user_map = Validation::sanitize_params_submitted( $_POST['user_map'][ $i ] );
			if ( ! empty( $user_map ) ) {
				$user = get_userdata( intval( $user_map ) );
				if ( isset( $user->ID ) ) {
					if ( $old_id ) {
				...
				$new_user = Validation::sanitize_params_submitted( $_POST['user_new'][ $i ] );
				if ( ! empty( $new_user ) ) {
					$user_id = wp_create_user( $new_user, wp_generate_password() );
	...
	function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( __( '%s &#8220;%s&#8221; already exists.', 'wordpress-importer' ), esc_html( $post_type_object->labels->singular_name ), esc_html( $post['post_title'] ) );
				echo '<br/>';
		...
		if ( 200 !== $remote_response_code ) {
			...
			return new WP_Error(
				...
				/* translators: 1: The HTTP error message. 2: The HTTP error code. */
					...
					esc_html( get_status_header_desc( $remote_response_code ) ),
	...
	function header() {
		...
		if ( isset( $updates[ $basename ] ) ) {
			...
			echo '<div class="error"><p><strong>';
			printf( esc_html__( 'A new version of this importer is available. Please update to version %s to ensure compatibility with newer export files.', 'wordpress-importer' ), esc_html( $update->update->new_version ) );
			echo '</strong></p></div>';
		}
	}
