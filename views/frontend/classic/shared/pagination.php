...
$maximum = count( $pages );
?>
	<nav class="realpress-pagination">
		<ul>
			<?php
			$pagination = '';
			if ( $current_page !== 1 ) {
				?>
				<li><a href="#" data-page="<?php echo esc_attr( $prev_page ); ?>"><i class="fas fa-angle-left"></i></a></li>
				<?php
			}
			for ( $i = 0; $i < $maximum; $i ++ ) {
				if ( intval( $current_page ) === intval( $pages[ $i ] ) ) {
					...
					?>
					<li class="disable"><span><i class="fas fa-ellipsis-h"></i></span></li>
					<?php
				} else {
					?>
			...
			if ( $current_page !== $max_page ) {
				?>
				<li><a href="#" data-page="<?php echo esc_attr( $next_page ); ?>"><i class="fas fa-angle-right"></i></a></li>
				<?php
			}
			?>
		</ul>
